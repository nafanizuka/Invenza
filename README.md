...
├── supabase.sql                 # Script SQL lengkap (tabel, RLS, storage, dummy data)
│
├── assets/
│   ├── images/
...
│
├── css/
│   ├── style.css                # Style global (tombol, modal, sidebar, dsb.)
...
└── js/
    ...
    ├── auth.js                     # requireAuth(), logout, session listener
    ├── login.js                    # Logika halaman login
    ├── signup.js                    # Logika halaman daftar
    ├── reset-password.js             # Logika lupa/reset password
    ├── dashboard.js                   # Logika statistik dashboard
    ├── products.js                     # Logika CRUD data barang
    └── product-detail.js                # Logika halaman detail barang
```
...
- Pengguna yang **sudah login** (`authenticated`) dapat **SELECT, INSERT, UPDATE, DELETE** semua data barang — karena aplikasi ini ditujukan untuk kolaborasi antar karyawan toko dalam satu inventaris bersama.
