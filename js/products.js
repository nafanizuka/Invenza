...
function renderTableRow(p) {
  ...
  const img = p.image_url
    ...
    : `<div class="product-thumb" style="display:flex;align-items:center;justify-content:center;color:var(--color-text-muted);">📦</div>`;
  return `
    <tr data-detail-id="${p.id}">
      ...
      <td>
        <div class="row-actions">
          ...
          <button class="btn-icon" data-action="delete" data-id="${p.id}" title="Hapus">🗑️</button>
        </div>
      </td>
    </tr>`;
}
...
async function isDuplicateCode(code, excludeId) {
  let query = supabaseClient.from("products").select("id").eq("code", code);
